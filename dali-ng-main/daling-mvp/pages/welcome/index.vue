...
export default {
  ...
  computed: {
    displayActivities() {
      // 有真实数据就展示真实数据，否则展示示例
      if (this.locationStore.hasPermission === true && this.activities.length > 0) {
        ...
        wx.getPrivacySetting({
          success: (res) => {
            if (res.needAuthorization) {
              // 用户还未同意隐私协议，弹窗让用户选择
              this.$refs.privacyPopup.authorize().then((agreed) => {
                resolve(agreed)
              })
            } else {
              // 已同意过，直接通过
